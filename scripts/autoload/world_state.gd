...
var body_rng_seed: int = 0
var body_rng_state: int = 0
var shift_rng_seed: int = 0
var shift_rng_state: int = 0
...
func reset_body_rng() -> void:
	body_rng_seed = int(world_seed) + 5000003
	var rng := RandomNumberGenerator.new()
	rng.seed = body_rng_seed
	body_rng_state = rng.state


func reset_shift_rng() -> void:
	shift_rng_seed = int(world_seed) + 6000003
	var rng := RandomNumberGenerator.new()
	rng.seed = shift_rng_seed
	shift_rng_state = rng.state


func reset_social_rng() -> void:
	social_rng_seed = int(world_seed) + 7000003
	var rng := RandomNumberGenerator.new()
	rng.seed = social_rng_seed
	social_rng_state = rng.state
...
func to_dict() -> Dictionary:
	...
	return {
		...
		"body_rng_seed": str(body_rng_seed),
		"body_rng_state": str(body_rng_state),
		"shift_rng_seed": str(shift_rng_seed),
		"shift_rng_state": str(shift_rng_state),
		"social_rng_seed": str(social_rng_seed),
		"social_rng_state": str(social_rng_state),
...
func load_dict(d: Dictionary) -> void:
	...
	if gossip_rng_state == 0:
		...
		rng.seed = gossip_rng_seed
		gossip_rng_state = rng.state
	body_rng_seed = _saved_int(d.get("body_rng_seed", int(world_seed) + 5000003),
			int(world_seed) + 5000003)
	body_rng_state = _saved_int(d.get("body_rng_state", 0), 0)
	if body_rng_state == 0:
		var rng := RandomNumberGenerator.new()
		rng.seed = body_rng_seed
		body_rng_state = rng.state
	shift_rng_seed = _saved_int(d.get("shift_rng_seed", int(world_seed) + 6000003),
			int(world_seed) + 6000003)
	shift_rng_state = _saved_int(d.get("shift_rng_state", 0), 0)
	if shift_rng_state == 0:
		var rng := RandomNumberGenerator.new()
		rng.seed = shift_rng_seed
		shift_rng_state = rng.state
	social_rng_seed = _saved_int(d.get("social_rng_seed", int(world_seed) + 7000003),
			int(world_seed) + 7000003)
	social_rng_state = _saved_int(d.get("social_rng_state", 0), 0)
	if social_rng_state == 0:
		var rng := RandomNumberGenerator.new()
		rng.seed = social_rng_seed
		social_rng_state = rng.state
	npcs.clear()
