...
4. **Click-to-move** on the player (NavigationAgent2D): click ground = walk, click Interactable = walk-then-interact. WASD unchanged.
   - Started 2026-06-12: `Player.tscn` has a `NavigationAgent2D`; click ground movement works and WASD cancels the path. Click-to-interact follow-through is implemented but still needs a stable interactive harness check.
