...
2. **World rebuild:** `tile_world.gd` switches to atlas TileSets; buildings become facade+roof props with Y-sort and collision (roof fades when the player walks behind); `town.gd` re-laid out organically — winding roads, plazas, alleys, district flavors (Downtown + the Bricks core; the diner/store/bar strip; Site 9/docks edge); props everywhere. `interior.gd` rooms get real floors, walls, and furniture sprites.
   ...
   - Continued 2026-06-12: exterior roof sprites now fade when the player stands behind a building roof band, and the scripted playtest verifies both fade and opacity restore. More organic road geometry, richer district clutter, and deeper facade composition remain.
   ...
   - Continued 2026-06-12: generated table, chair, and plant props now dress interiors as non-colliding sprites, with scripted checks for diner/store decor density. Full-room composition, interior wall depth, and richer location-specific clutter remain.
   ...
   - Continued 2026-06-12: exterior roads now carry lightweight ambient traffic cars on deterministic routes, with scripted playtest coverage that the cars spawn and move. NPC-owned driving routines and traffic collision remain.
...
- Origin opening beats (arrive by bus / wake behind the gas station…), HUD goal tracker fed by the existing `LifePaths`, tutorialized first day.
  ...
  - Continued 2026-06-12: origin opening beats now use an authored `opening_line` on `OriginDef`, with all six shipped origins covered by the real-scene opening smoke test.
...
- In-world verbs: pickpocket (proximity/behind), shoplifting under clerk sightlines, street carjacks; fence/dealer as scenes.
  ...
  - Continued 2026-06-12: empty parked-car theft now uses a dedicated car-theft helper with the design doc's hot-plate risk: hot plates force warrant-grade evidence, infamy, and a Gazette entry even if nobody saw the door open.
  - Continued 2026-06-12: dealer spots now open the shared shop UI with black-market context instead of inheriting QuikStop-only Pocket/register robbery controls, and dealer purchases prefer dirty cash. Store and dealer payloads are covered in the scripted playtest.
  - Continued 2026-06-12: witness intervention has started with dialogue-driven bribes and intimidation: witnesses to active player crimes expose bribe/pressure actions, success suppresses report evidence, and failure can create linked coverup charges.
- Cops visibly patrol; wanted = embodied pursuit (chase AI) → arrest confrontation; jail becomes a real interior with a daily event loop (yard / library / kitchen) instead of a toast.
  ...
  - Continued 2026-06-12: exterior warrant stops now show a generated police car driving into the scene before the arrest confrontation opens. This makes the response visible in the world; pursuit/pathing AI and interior-to-exterior staging still remain.
  ...
  - Continued 2026-06-12: accepted arrest bribes now let corrupt cops mishandle active warrant evidence, potentially dropping evidence below warrant grade and clearing wanted stars. Full corrupt-cop contact services remain.
  - Continued 2026-06-12: Rust Harbor County Jail now exists as a signed exterior building and walkable cell/booking interior. Serving a sentence through the real arrest UI transitions into `loc_jail` and leaves the player in the cell scene after the sentence resolves. Interactive jail day choices, guards, and inmate routines remain.
